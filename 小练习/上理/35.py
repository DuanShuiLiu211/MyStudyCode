...
class Noises:
    ...
    def add_normal_noise(self):
        noise = (
            np.random.normal(self.mean, self.sigma, self.image.shape) * 255
        ).astype(np.uint8)
        return noise
    ...
    def add_normal_poisson_noise(self):
        noise = (self.add_normal_noise() + self.add_poisson_noise()).astype(np.uint8)
        return noise
...
def cropper(
    image, crop_shape, anchor_shape=None, jitter=False, max_jitter=None, planes=None
):
    half_crop_shape = tuple(_c // 2 for _c in crop_shape)
    if anchor_shape is None:
        anchor_shape = tuple(_i // 2 for _i in image.shape)
    assert all(
        [_i - _c >= 0 for _c, _i in zip(half_crop_shape, anchor_shape)]
    ), "Crop shape is bigger than image shape"

    if jitter:
        contrain_1 = tuple(
            (_i - _c) // 4 for _c, _i in zip(half_crop_shape, anchor_shape)
        )
        ...
        all(
            [
                ...
                for _m, _i in zip(max_jitter, anchor_shape)
            ]
        ) or present(
            ValueError(
        ...
        loc = tuple(
            _l - np.random.randint(-1 * max_jitter[_i], max_jitter[_i] + 1)
