...
class npccLoss:
    ...
    def __call__(self, pred, target):
        ...
        cost = -K.sum((vpred * vtarget)) / (
            K.sum(K.sqrt((vpred**2))) * K.sum(K.sqrt((vtarget**2))) + 1e-8
        )
...
class DSSIMObjective:
    ...
    def __call__(self, pred, target):
        ...
        patches_pred = KC.extract_image_patches(
            pred, kernel, kernel, "valid", self.dim_ordering
        )
        patches_target = KC.extract_image_patches(
            target, kernel, kernel, "valid", self.dim_ordering
        )
        ...
        # Get std dev
        covar_target_pred = (
            K.mean(patches_target * patches_pred, axis=-1) - u_target * u_pred
        )

        ssim = (2 * u_target * u_pred + self.c1) * (2 * covar_target_pred + self.c2)
        denom = (K.square(u_target) + K.square(u_pred) + self.c1) * (
            var_pred + var_target + self.c2
        )
        ssim /= denom
