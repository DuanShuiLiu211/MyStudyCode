...
def overlap_area(polygon1, polygon2, samples=10000):
    ...
    # 计算两个边界框的交集
    bb_overlap = (max(bb1[0], bb2[0]), max(bb1[1], bb2[1]), min(bb1[2], bb2[2]), min(bb1[3], bb2[3]))
    ...
    for _ in range(samples):
        ...
        if is_point_inside_polygon((x, y), polygon1) and is_point_inside_polygon((x, y), polygon2):
            count_inside_both += 1

    return count_inside_both / samples * (bb_overlap[2] - bb_overlap[0]) * (bb_overlap[3] - bb_overlap[1])
...
def overlap_area(coords1, coords2):
    # 将坐标列表转换为点元组的列表
    polygon1 = Polygon([(coords1[i], coords1[i + 1])
                        for i in range(0, len(coords1), 2)])
    polygon2 = Polygon([(coords2[i], coords2[i + 1])
