...
foldlist = set(glob('/home/gyc/DL_learning/microport/视频分割-final/第三批/*')) - set(
    glob('/home/gyc/DL_learning/microport/视频分割-final/第三批/*.png')) - set(
        glob('/home/gyc/DL_learning/microport/视频分割-final/第三批/*.avi'))
...
for i in range(len(foldlist)):
    ...
    if len(frames) > 0:
        start_frame = int(frames[0]) - 1 if int(frames[1]) > int(
            frames[0]) else int(frames[1])
        end_frame = int(frames[0]) - 1 if int(frames[0]) > int(
            frames[1]) else int(frames[1])
        ...
        for k in range(len(imgList)):
            if (int(imgList[k].split('/')[-1].replace('.png', '')) >=
                    start_frame) & (int(imgList[k].split('/')[-1].replace(
                        '.png', '')) <= end_frame):
                shutil.copy(imgList[k],
                            imgList[k].replace('第三批', '第三批颈总动脉视频截取'))
