...
def plot_ann(number_input, number_hidden, number_output):
    ...
    edge_input_hidden_list.extend(
        [(i, j) for i in vertex_input_list for j in vertex_hidden_list[0]]
    )  # 输入层-隐藏层
    edge_list = []
    ...
    if ceng_hidden > 1:
        for k in range(ceng_hidden - 1):
            edge_hidden_hidden_list.extend(
                [
                    (i, j)
                    for i in vertex_hidden_list[k]
                    for j in vertex_hidden_list[k + 1]
                ]
            )  # 隐藏层-隐藏层
    ...
    edge_hidden_output_list.extend(
        [
            (i, j)
            for i in vertex_hidden_list[len(vertex_hidden_list) - 1]
            for j in vertex_output_list
        ]
    )  # 隐藏层-输出层
    ...
    # 位置
    pos = {}
    ceng_pos_x = np.linspace(
        -(ceng_hidden + 2) / 2, (ceng_hidden + 2) / 2, num=ceng_hidden + 2
    )
    list(
        map(
            lambda i: pos.update(
            ...
            np.arange(-number_input / 2 * 1 + 1 / 2, number_input / 2 * 1 + 1 / 2, 1),
        )
    )  # 输入层
    list(
        map(
            lambda j: list(
                map(
                    lambda i: pos.update(
                        {
                            vertex_hidden_list[j][
                                int(
                                    np.where(
                                        np.arange(
                                            -number_hidden[j] / 2 * 1 + 1 / 2,
                                            number_hidden[j] / 2 * 1 + 1 / 2,
                                            1,
                                        )
                                        == i
    ...
    list(
        map(
            ...
            np.arange(-number_output / 2 * 1 + 1 / 2, number_output / 2 * 1 + 1 / 2, 1),
        )
    )  # 输出层
    ...
    plt.ylim(
        -max(number_input, max(number_hidden), number_output) / 2 * 1,
