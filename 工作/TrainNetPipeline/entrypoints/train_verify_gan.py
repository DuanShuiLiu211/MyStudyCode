            ...
            if (e + 1) % int(cfg['epoch_print']) == 0:
                if (idx + 1) % int(cfg['batch_print']) == 0:
                    if cfg['save_train_image']:
                        save_tensor_to_image(cfg['train_result_save_dir_generator'], e, fn,
        ...
        for idx, (data, target, fn) in enumerate(tsld):
            ...
            if (e + 1) % int(cfg['epoch_print']) == 0:
                if cfg['save_test_image']:
                    save_tensor_to_image(cfg['test_result_save_dir_generator'], e, fn, g_data_f, loss_g1)
...
if __name__ == '__main__':
    ...
    g_model = get_model(generator_config, device, logger)
    d_model = get_model(discriminator_config, device, logger)
