...
def get_model(model_config, device, logger):
    # 选择需要使用的模型
...
def load_train_verify_model(model_config, model, optimizer, epoch_current, device, logger):
    logger.info(model)
    count_model_complexity(model, device, logger)
    load_mode = model_config['model_load_mode']
    model_weight = model_config['train_verify_model_weight'] 
    try:
        if load_mode == "model":
            ...
            new_weight_dict = torch.load(model_weight['model_state_dict'], map_location=device)
            optimizer.load_state_dict(model_weight['optimizer_state_dict'])
            epoch_current = model_weight['epoch_current']
        else:
            new_weight_dict = {}
    ...
    return model, optimizer, epoch_current


def load_test_model(model_config, model, device, logger):
    logger.info(model)
    count_model_complexity(model, device, logger)
    load_mode = model_config['model_load_mode']
    model_weight = model_config['test_model_weight'] 
    try:
        if load_mode == "model":
