...
def tes_process(model, test_loader, device):
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            outputs = cv2.resize(
                outputs, (origin_size[1].item(), origin_size[0].item()),
...
def result_show(i, img, pred, truth, image_name):
    ...
    cv2.imencode('.png',
                 img)[1].tofile(os.path.join(save_dir, f'{image_name[0]}.png'))
...
def cal_dice(i, pred, truth, spacing):
    ...
    if 0 == np.count_nonzero(pred_both):
        ...
        asd_both = medmetric.binary.asd(pred_both,
                                        truth_both,
                                        voxelspacing=spacing)
        assd_both = medmetric.binary.assd(pred_both,
                                          truth_both,
                                          voxelspacing=spacing)
        hd_both = medmetric.binary.hd(pred_both,
                                      truth_both,
                                      voxelspacing=spacing)
        hd_both95 = medmetric.binary.hd95(pred_both,
                                          truth_both,
                                          voxelspacing=spacing)
    ...
    return 100 * dice_in, 100 * dice_both, 100 * iou_in, 100 * iou_both, 100 * recall_in, 100 * recall_both, 100 * precision_in, 100 * precision_both, asd_in, asd_both, assd_in, assd_both, hd_in, hd_both, hd_in95, hd_both95
...
if __name__ == '__main__':
    ...
    for i in range(len(prob)):
        spacing = int(float(
            pixels[np.argwhere(names == image_names[i])][0][0]))
        # di, do, db, ii, io, ib, pn, tn = cal_dice(i, prob[i], truth[i])

        dice_in, dice_both, iou_in, iou_both, recall_in, recall_both, precision_in, precision_both, \
        asd_in, asd_both, assd_in, assd_both, hd_in, hd_both, hd_in95, hd_both95 = \
