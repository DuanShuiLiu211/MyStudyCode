    ...
    for i in range(len(data_list)):
        ...
        if len(data_img_label) <= 4:
            ...
            if flag == 1:
                ...
                point_list_r1 = np.vstack(
                    (front_inpoints[::-1], front_outpoints))
                point_list_g1 = np.vstack(
                    (back_inpoints[::-1], front_inpoints))
                point_list_r2 = np.vstack(
                    (back_inpoints[::-1], back_outpoints))
                ...
                # 填充
                cv2.fillPoly(label_img, pts=[point_list_r1],
                             color=(0, 0, 255))  # R
