...
if __name__ == '__main__':
    ...
    for i in range(len(label_list)):
        ...
        if (len(CCA_MAB_points)!=0) & (len(CCA_lumen_points)!=0):
            cv2.fillPoly(label_img, pts=[adjust_pts_order(CCA_MAB_points)], color=(0, 0, 255))
            cv2.fillPoly(label_img, pts=[adjust_pts_order(CCA_lumen_points)], color=(0, 255, 0))
        
        if (len(IJV_points)!=0):
            cv2.fillPoly(label_img, pts=[adjust_pts_order(IJV_points)], color=(255, 0, 0))

        if (len(BIF_lumen_points)!=0) & (len(BIF_MAB_points)!=0):
            cv2.fillPoly(label_img, pts=[adjust_pts_order(BIF_MAB_points)], color=(16, 16, 16))
            cv2.fillPoly(label_img, pts=[adjust_pts_order(BIF_lumen_points)], color=(16*2, 16*2, 16*2))

        if (len(ICA_lumen_points)!=0) & (len(ICA_MAB_points)!=0):
            cv2.fillPoly(label_img, pts=[adjust_pts_order(ICA_MAB_points)], color=(16*3, 16*3, 16*3))
            cv2.fillPoly(label_img, pts=[adjust_pts_order(ICA_lumen_points)], color=(16*4, 16*4, 16*4))

        if (len(ECA_lumen_points)!=0) & (len(ECA_MAB_points)!=0):
            cv2.fillPoly(label_img, pts=[adjust_pts_order(ECA_MAB_points)], color=(16*5, 16*5, 16*5))
            cv2.fillPoly(label_img, pts=[adjust_pts_order(ECA_lumen_points)], color=(16*6, 16*6, 16*6))
