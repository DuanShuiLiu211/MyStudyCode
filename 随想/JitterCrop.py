...
def cropper(image, crop_shape, jitter=False, max_jitter=None, planes=None):
    ...
    if jitter:
        contrast_1 = tuple((_i - _c) // 4 for _c, _i in zip(half_crop_shape, half_image_shape))
        contrast_2 = tuple(c // 2 for c in half_crop_shape)
        if max_jitter is None:
            max_jitter = tuple([min(_ct2, _ct1) for _ct2, _ct1 in zip(contrast_1, contrast_2)])
        ...
        loc = tuple(_l - np.random.randint(-1 * max_jitter[_i], max_jitter[_i]) for _i, _l in enumerate(half_image_shape))
    else:
