...
if __name__ == '__main__':
    """一对二的数据标签"""
    # 数据集名称
    base_dir = 'dataset_gm_3_fixed_20_512_1024'
    data_folder = f'{base_dir}/data'
    label_folder = f'{base_dir}/label'
    otherLabel_folder = f'{base_dir}/aberration'

    # 标签文件名称
    all_sample_txt = 'label_01.txt'
    train_txt = 'label_train_01.txt'
    test_txt = 'label_test_01.txt'
    verify_txt = 'label_verify_01.txt'
    ...
    label_folderList = os.listdir(label_folder)
    file_remove(label_folderList)
    otherLabel_folderList = os.listdir(otherLabel_folder)
    file_remove(otherLabel_folderList)

    print(f'The number of labels is: {len(label_folderList)}.')
    ...
    for file in input_folderList:
        ...
        if os.path.isdir(os.path.join(data_folder, file)):
            ...
            label_list = os.listdir(os.path.join(label_folder, file))
            otherLabel_list = os.listdir(os.path.join(otherLabel_folder, file))
            print(f'The number of {file} is: {len(data_list)}.')

            for data_fn in data_list:
                if data_fn in label_list and data_fn.replace(".tif", ".npy") in otherLabel_list:
                    sample_list.append('{} {} {}\n'.format(os.path.join(data_folder, file, data_fn),
                                                           os.path.join(label_folder, file, data_fn),
                                                           os.path.join(otherLabel_folder, file, data_fn.replace(".tif", ".npy"))))
                else:
                    print(f'Error! The {data_fn} in folder: {file} cannot be found in folder: {label_folder}.')
                    sys.exit()
        else:
            if file in label_folderList:
                sample_list.append('{} {} {}\n'.format(os.path.join(data_folder, file),
                                                       os.path.join(label_folder, file),
                                                       os.path.join(otherLabel_folder, file)))
    ...
    print(f'The {all_sample_txt}, have been constructed!')
    ...
    if all_random:
        ...
        verify_index = index_list[int(all_sample_len * train_ratio):]
